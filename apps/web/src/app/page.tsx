  ...
  return (
    <main className="min-h-screen bg-slate-950">
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(124,58,237,0.18),transparent_18%)]" />
        <div className="mx-auto max-w-7xl px-6 pb-24 pt-16 lg:px-8">
          <nav className="flex items-center justify-between rounded-2xl border border-slate-800 bg-slate-900/50 px-4 py-3 backdrop-blur">
            ...
            <div className="flex gap-3">
              <Link href="/login" className="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-200">Iniciar sesión</Link>
              <Link href="/register" className="rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2 text-sm font-medium text-white">Crear cuenta</Link>
            </div>
          </nav>
          ...
          <div className="mt-16 grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              ...
              <h1 className="mt-4 text-5xl font-semibold leading-tight text-white lg:text-6xl">Controla tus criptoactivos con confianza.</h1>
              <p className="mt-5 max-w-2xl text-lg text-slate-300">Envía, recibe y monitorea tus fondos desde una interfaz moderna, segura y fácil de usar.</p>
              <div className="mt-8 flex gap-3">
                <Link href="/register" className="rounded-2xl bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-3 font-medium text-white">Comenzar ahora</Link>
                <Link href="/dashboard" className="rounded-2xl border border-slate-700 px-6 py-3 font-medium text-slate-100">Ver demo</Link>
              </div>
              <ul className="mt-8 flex flex-wrap gap-3">
                {features.map((feature) => (
                  <li key={feature} className="rounded-full border border-slate-700 bg-slate-900/70 px-4 py-2 text-sm text-slate-300">{feature}</li>
                ))}
              </ul>
            </div>
